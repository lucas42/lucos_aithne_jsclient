...
- **`aithne.verifySession(cookieHeader, { requiredScope })` → `Promise<Classification>`** — the primary entry point. Extracts the `aithne_session` cookie, verifies the JWT (ES256-pinned, issuer/audience/skew per contract §"Verify the signature"/§"Validate standard claims"), applies serve-stale on JWKS infra failure, and applies the scope check.
- **`aithne.verifyToken(rawToken)` → `Promise<Classification>`** — same verification without cookie extraction or the scope gate (for callers that already hold a token, e.g. a WebSocket handshake that reads the cookie itself).
- **`aithne.parseCookies(header)` → object** — exported utility (cookie values may contain `=`).
- **`aithne.loginUrl(returnUrl)` → string** — builds `${origin}/auth/login?next=${encodeURIComponent(returnUrl)}`. The consumer decides `returnUrl` (it MUST derive it from a non-user-controllable base such as `APP_ORIGIN`, never the raw `Host` header — an open-redirect guard the library documents but cannot enforce, since it does not see the request).

**`requiredScope`** accepts a string or array; the library grants when the token's `scopes` contains it, and additionally accepts the estate-wide `render-ui` scope **only when `ENVIRONMENT === 'development'`** (contract §"Development-only `render-ui` bypass"). For consumers needing a read/write-split or an identity (`sub`) check (contract §6), an `{ authorize: (payload) => boolean }` predicate is accepted as an escape hatch instead of `requiredScope`; the library still classifies authentication/availability and defers only the grant decision.
...
// Classification
{
  ...
  error:   <Error | null>,         // present for 'unavailable' and non-infra 'unauthenticated'
}
```
...
- **Latent divergences get normalised** on adoption — e.g. `lucos_creds`' `Host`-header-based `next=` (a latent open-redirect-shaped smell) is replaced by the library's `loginUrl` helper fed an `APP_ORIGIN`-derived return URL.
